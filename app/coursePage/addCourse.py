...
from app import db
from app.models import Course,Manager,Teacher,CourseForTeacher,Klass,Lesson,Record,Student
...
@coursePage.route('/courseforteacher/add',methods = ['POST'])
@tokenUtils.token_required
		...
		if teacher is None:
			...
			if klass_info is None or course_id is None or weekDate is None\
				...
				try:
					...
					aKlass = Klass.query.filter_by(belonged_college=college,belonged_grade=grade,name=name).first()
					klass_id = aKlass.id
					...
					status = 1
					courseforteacher = CourseForTeacher(course_id, teacher_id,klass_id,start_date,end_date,week_date,status)
